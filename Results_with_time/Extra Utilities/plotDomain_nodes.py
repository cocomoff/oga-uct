...
plt.xlabel('Number of nodes',fontsize=20, labelpad = 20)
plt.ylabel('Accumulated Cost',fontsize=20, labelpad = 20)#'large')

plt.errorbar(base[:,0], base[:,1], yerr = 2*base[:,2], color='blue', linewidth = 2, label = "Vanilla")
plt.errorbar(asap[:,0], asap[:,1], yerr = 2*asap[:,2], color='red', linewidth = 2, label = "ASAP")
plt.errorbar(k1[:,0], k1[:,1], yerr = 2*k1[:,2], color='goldenrod', linewidth = 2, label = "K=1")
#plt.errorbar(k2[:,0], k2[:,1], yerr = 2*k2[:,2], color='violet', linewidth = 2, label = "K=2")
plt.errorbar(k3[:,0], k3[:,1], yerr = 2*k3[:,2], color='green', linewidth = 2, label = "K=3")
...
plt.title(domain + ' with nodes', fontsize = 30, y=1.02)#sys.argv[4])
