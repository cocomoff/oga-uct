function calc_normalized_score(domain,count)
...
for m=0:count
    ...
    score_mat(:,2)=asapReg(:,2);
    score_mat(:,3)=ogak1Reg(:,2);
    for i=1:6
        if(ogak1Reg(i,1)>asapReg(i,1))
            if(ogak1Reg(i,1)>baseReg(i,1))
                score_mat(i,3)=0;
                if(asapReg(i,1)>baseReg(i,1))
                    score_mat(i,2)=(asapReg(i,1)-baseReg(i,1))/(ogak1Reg(i,1)-baseReg(i,1));
                    score_mat(i,1)=1;
                else
                    score_mat(i,1)=(baseReg(i,1)-asapReg(i,1))/(ogak1Reg(i,1)-asapReg(i,1));
                    score_mat(i,2)=1;
                end
            else
                score_mat(i,3)=(ogak1Reg(i,1)-asapReg(i,1))/(baseReg(i,1)-asapReg(i,1));
                score_mat(i,2)=1;
                score_mat(i,1)=0;
            end
        else
            if(ogak1Reg(i,1)<baseReg(i,1))
                ...
                if(asapReg(i,1)>baseReg(i,1))
                    score_mat(i,2)=0;
                    score_mat(i,1)=(baseReg(i,1)-ogak1Reg(i,1))/(asapReg(i,1)-ogak1Reg(i,1));
                else
                    score_mat(i,1)=0;
                    score_mat(i,2)=(asapReg(i,1)-ogak1Reg(i,1))/(baseReg(i,1)-ogak1Reg(i,1));
                end
            else
                score_mat(i,3)=(ogak1Reg(i,1)-baseReg(i,1))/(asapReg(i,1)-baseReg(i,1));
                score_mat(i,2)=0;
                score_mat(i,1)=1;
            end
        end
